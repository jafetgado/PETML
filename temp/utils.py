...
def mafft_MSA(read_path, write_path, mafft_exec):
...
HMMALIGN_EXEC = '/projects/bpms/jgado/hmmer-3.2.1/src/hmmalign'
...
def mapSeqToMSA(full, aln):
    '''Return an array that maps positions in an aligned sequence to a full length sequence
    which may flank the aligned sequence to the left or right. The first column of the
    array represents the positions in the full sequence, and the second column represents
    the positions in the aligned sequence. Gap positions are indicated by NaN values.'''

    trunc = aln.replace('-', '')  # Aligned sequence without gaps
    full_store = []

    for aln_pos, aln_char in enumerate(aln):

        # Gaps are represented by NaNs
        if aln_char == '-':
            full_pos = np.nan
        
        else:
            # Region in full sequence
            before, after = aln[:aln_pos].replace('-',''), aln[aln_pos:].replace('-','')
            if len(before) != (len(trunc) - 1):
                full_pos = full.index(after)
            else:
                full_pos = full_pos + 1 # For the last position

        # Save position
        full_store.append(full_pos)

    # Mapping array
    aln_to_seq = np.array([full_store, np.arange(len(full_store))]).transpose()
...
def alignWithMapping(query, aln_to_seq):
    '''Convert a mutant sequence (query) to an aligned sequence by using the mapping
    array aln_to_seq'''

    query = np.array(list(query), dtype=np.object)
    aligned = np.array(list('-' * len(aln_to_seq)), dtype=np.object)
    aln_to_seq_nogap = aln_to_seq[~np.isnan(aln_to_seq[:,0]),:]
    
    query_locs = aln_to_seq_nogap[:,0].astype(np.int)
    align_locs = aln_to_seq_nogap[:,1].astype(np.int)
    
    aligned[align_locs] = query[query_locs]
...
def confIntRho(x, y, n=1000):

    assert len(x) == len(y)
    x, y = np.asarray(x), np.asarray(y)
    rhos = []
    for i in range(n):
        locs = np.random.choice(range(len(x)), size=len(x), replace=True)
        rhos.append(spearmanr(x[locs], y[locs])[0])

    return (round(np.percentile(rhos, 5), 3),
            round(spearmanr(x, y)[0], 3),
            round(np.percentile(rhos,95), 3))
